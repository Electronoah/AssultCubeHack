...
DWORD AmmoOffsets[] = { 0x384, 0x14, 0x0 }; //3 LEVEL pointer
...
void WriteToMemory(HANDLE hProcHandle)
{
	...
	if (HealthStatus)
	{
		//can also cast to LPVOID 
		//ex:WriteProcessMemory(hProcHandle, (LPVOID)HealthAddressToWrite, &HealthValue, sizeof(HealthValue), NULL); 
